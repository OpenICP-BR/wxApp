...
bool CertsPanelClass::UnlockCert(wxString pass) {
	...
	if (err_code == OK) {
		...
		XRCCTRL(*wizAddCert, "outCertName", wxStaticText)->SetLabel(new_cert.subject.common_name);
		XRCCTRL(*wizAddCert, "outCertEmail", wxStaticText)->SetLabel(new_cert.subject.email);
		XRCCTRL(*wizAddCert, "outCertCPF", wxStaticText)->SetLabel(new_cert.subject.doc_id);
		XRCCTRL(*wizAddCert, "outCertAC", wxStaticText)->SetLabel(new_cert.issuer.common_name);
		return true;
	} else if (err_code == 1) {
